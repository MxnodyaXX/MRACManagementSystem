...
export default function Owners() {
  ...
  return (
    <div>
      <Header title="Owner Dashboard" subtitle="Per-owner fleet, earnings, and payout summary" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
        {owners.map((o) => {
          ...
          const ownerBookings = bookings.filter((b) =>
            ownerVehicles.some((v) => v.id === b.vehicleId)
          );
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-xl bg-navy-700 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                  {o.name.split(' ').map((n) => n[0]).join('').slice(0, 2)}
                </div>
                <div>
                  <p className="text-sm font-bold text-navy-800">{o.name}</p>
                  <p className="text-xs text-navy-400">{o.phone}</p>
                  {o.email && <p className="text-xs text-navy-400">{o.email}</p>}
                </div>
              ...
              <div className="space-y-1.5 mb-3">
                {ownerVehicles.map((v) => (
                  <div key={v.id} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-navy-600 flex-1">{v.brand} {v.model} · {v.vehicleNumber}</span>
